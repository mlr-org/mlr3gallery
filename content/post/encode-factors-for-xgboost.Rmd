...
author: Michel Lang
date: '2020-01-31'
slug: encode-factors-for-xgboost
categories: []
tags: ['classification', 'factor encoding', 'mlr3pipelines']
packages: ['mlr3', 'mlr3learners', 'mlrpipelines', 'xgboost']
---
...
The package `r cran_pkg("xgboost")` unfortunately does not support handling of factor levels.
Therefore it is required to manually convert factor features to numerical dummy features.
...
First, we create an example task with factors (`r ref("mlr_tasks_german_credit", "german_credit")`) and the `r ref("mlr_learners_classif.xgboost", "xgboost learner")`:
```{r}
library(mlr3)
...
Success! We augmented xgboost with handling of factor and ordered factor levels.
