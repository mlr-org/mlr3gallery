...
slug: date-features
categories: []
tags: ['feature-engineering', 'mlr3pipelines', 'random-forest', 'regression', 'date-features']
...
handle a cyclical feature $\mathbf{x}$ is to compute the sine and cosine transformation of $\frac{2
\pi \mathbf{x}}{\mathbf{x}_{\text{max}}}$, with $\mathbf{x}_{\text{max}} = 24$ for hours and $60$ for minutes and seconds. This results in a two-dimensional representation of the
feature:
...
plot(hours_sin, hours_cos, type = "n", xlab = "Sine Transformation", ylab = "Cosine Transformation")
...
This is useful as most learners naturally cannot handle dates and `POSIXct` variables and therefore
require conversion prior to training.
...
corresponding weather and seasonal information. The dataset can be downloaded from the UCI Machine
Learning Repository. After reading in the data, we fix some factor levels, and convert some data
types:
```{r}
tmp <- tempfile()
...
Note that this involves validating on past data, which is usually a bad practice but should suffice
for this example:
```{r}
cv3 = rsmp("cv", folds = 3)
```

As our baseline model, we use a random forest, ranger learner.  For the baseline, we only use the
original features that are sensible and drop `instant` (record index), `dteday` (year-month-day as a
`character`, not usable) and `date` (our new `POSIXct` variable which we will only use later). We
also do not use `casual` (count of casual users) and `registered` (count of registered users) as
features as they together add up to `cnt` and could be used as different target variables if we were
interested in only the casual or registered users.
```{r}
...
`POSIXct` columns of the data and by default adds plenty of new date-time related features. Here, we
want to add all except for `minute` and `second`, because this information is not available. As we
additionally want to use cyclical versions of the features we set `cyclic = TRUE`:
```{r}
...
our training task, we can now drop the features, `yr`, `mnth`, `hr`, and `weekday`, because our
...
Note that it may be useful to familiarize yourself with `PipeOpRemoveConstants` which can be used
after the feature engineering to remove features that are constant.  `PipeOpDateFeatures` does not
...
We create a `r ref("GraphLearner")` consisting of the `PipeOpDateFeatures` pipeline and a ranger
learner. This `GraphLearner` then behaves like any other `Learner`:
```{r}
