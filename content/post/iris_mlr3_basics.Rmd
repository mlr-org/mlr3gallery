...
Lets do usual ML operations: Train on some observations, predict on others.
...
This includes data type, constraints, defaults, etc.
...
If we want more learners, we can load on extension package from mlr3's
...
```{r}
learners = list(learner1, learner2, lrn("classif.rpart"))
...
We can visualize the performance of a learner using the `mlr3viz`package.

```{r}
task = tsk("iris")$select(c("Sepal.Length", "Sepal.Width"))
resampling = rsmp("cv", folds = 3)
object = resample(task, learner2, resampling, store_models = TRUE)
