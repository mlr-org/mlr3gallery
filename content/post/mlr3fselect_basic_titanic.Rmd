...
selection is to enhance the interpretability of models, speed up the learning
...
In an evolutionary search, feature sets with a high predictive performance
survive and generate new offspring feature sets by combination and mutation.
We have to define the population and offspring parameter `mu` and `lambda`.
...
We retrieve the best performing feature set.
...
The feature sets with all features except `age` achieved the highest predictive performance.
We may see a different result if we replace the `3-fold-cross-validation` with a
cross validation with many repetitions since the used resampling instance
introduces much randomness.
