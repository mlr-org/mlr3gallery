...
packages: ['OpenML', 'mlr3', 'mlr3learners', 'mlr3pipelines', 'mlr3tuning', 'mlr3measures', 'mlr3viz']
---
...
We use `AutoTuner` to tune the pipeline (random forest learner + imbalancy correction method) based on a 3-fold CV using the `classif.fbeta` as performance measure. 
...
Note that SMOTE has two hyperparameters `K` and `dup_size` where `K` changes the behavior of the SMOTE algorithm and `dup_size` affects oversampling rate. 
Hence, SMOTE with K = 1 can be seen as a different imbalacy correction method as SMOTE with K=2 (and so on).
We use grid search with 5 different hyperparameter configurations for the undersampling method, the oversampling method and each SMOTE variant for tuning:
