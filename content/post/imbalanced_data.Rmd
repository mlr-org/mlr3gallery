...
We mainly focus on undersampling the majority class, oversampling the minority class, and the SMOTE imbalance correction that enriches the minority class with synthetic data. 
...
table(po_under$train(list(task))$output$truth()) # reduce majority class by factor '1/ratio'
...
table(po_over$train(list(task))$output$truth()) # enrich minority class by factor 'ratio'
...
To keep runtime low, we define the search space only for the imbalacy correction method. 
...
To focus on the effect of the oversampling rate on the performance, we will consider SMOTE with K = 2 as a different imbalacy correction method as SMOTE with K = 4 (and so on).
