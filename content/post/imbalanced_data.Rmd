...
categories: []
tags: ['mlr3', 'mlr3pipelines', 'mlr3pipelines', 'basics', 'visualization']
...
The use case requires prior knowledge in basic ML concepts (e.g., issues imbalanced data, hyperparameter tuning, nested cross-validation) and `mlr3`, `mlr3pipelines` and `mlr3tuning` is required.
...
* Define imbalancy correction pipelines (undersampling, oversampling and smote) with `mlr3pipelines`  
...
library("mlr3measures")  # lots of performance measures
...
The OpenML R package can query available data sets and download them directly from within an R session:
...
We use `AutoTuner` to tune the pipeline (random forest learner + imbalancy correction method) based on a 3-fold CV using the `classif.f_score` as performance measure. 
As we defined the search space only for the imbalacy correction method, we will only tune their hyperparameter to keep the computational runtime low.
However, we could also jointly tune the hyperparameter of the learner along with the imbalancy correction method. 
We use grid search with 8 different hyperparameter configurations:

```{r}
# define the 
inner_cv3 = rsmp("cv", folds = 3)
measure = msr("classif.f_score")
...
For the tuning, we used 3-fold CV.
Now, we use the `benchmark` function to compare the tuned class imbalancy pipelines based on a 5 times repeated subsampling for the outer evaluation:
...
# compute and store benchmark result (or load it from file if it already exists)
if (!file.exists("imbalanced_data_bench.Rds")) {
  set.seed(1)
  # future::plan("multiprocess")
  bmr = benchmark(design, store_models = TRUE)
  saveRDS(bmr, file = "imbalanced_data_bench.Rds")
} else {
  bmr = readRDS("imbalanced_data_bench.Rds")
}
```
...
```{r}
library(ggplot2)
utune_path = bmr$learners$learner[[1]]$archive("params")
utune_gg = ggplot(utune_path, aes(x = undersample.ratio, y = classif.f_score)) +
  geom_point(size = 3) + 
  geom_line() + ylim(0.9, 1)

otune_path = bmr$learners$learner[[2]]$archive("params")
otune_gg = ggplot(otune_path, aes(x = oversample.ratio, y = classif.f_score)) +
  geom_point(size = 3) + 
  geom_line() + ylim(0.9, 1)

stune_path = bmr$learners$learner[[3]]$archive("params")
stune_gg = ggplot(stune_path, aes(x = smote.dup_size, y = classif.f_score, col = factor(smote.K))) +
  geom_point(size = 3) + 
  geom_line() + ylim(0.9, 1)

library(gridExtra)
grid.arrange(utune_gg, otune_gg, stune_gg, nrow = 1)
```
