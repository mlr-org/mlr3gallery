...
FIXME: reference links for new PipeOps etc. when new mlr3pipelines hits CRAN
...
Internally, this function will be applied to the `$data$tab` slot of a `r ref("Prediction", "Prediction")` object without the `$row_id` and `$truth` columns, and we specify that the `$response` column should be transformed.

To actually carry out the inverse transformation function after predicting from the `Learner`, we then rely on `PipeOpTargetInvert`. `PipeOpTargetInvert` has an empty `r ref("ParamSet", "ParamSet")` and its sole purpose is to apply the inverse transformation function after having predicted from a `Learner` (note that this whole design of target transformations may seem somewhat over-engineered, however, we will learn of its advantages when we later move to the advanced section).
...
We will talk more about the technical details here in the advanced section. For now, to finally construct our target transformation pipeline, we build a `Graph`:
...
FIXME: change the following sentence once predict type can be passed along

We can then finally call `$train()` and `$predict()` (prior to this we wrap the `Graph` in a `r ref("GraphLearner", "GraphLearner")` and here we do have to set the `task_type` manually, because `PipeOpTargetInvert` does not expose its prediction type):
...
```{r, eval = FALSE, tidy = FALSE}
    .invert = function(prediction, predict_phase_state) {
      ...
      response = predict(predict_phase_state$bc, newdata = prediction$response,
        inverse = TRUE)
      invoke(get(mlr_reflections$task_types["regr"]$prediction)$new,
