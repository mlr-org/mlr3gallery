...
```{r target-transformations-via-pipelines-016}
...
```{r target-transformations-via-pipelines-018, echo = FALSE, out.width = "100%"}
...
```{r target-transformations-via-pipelines-019}
library(R6)
```
...
```{r target-transformations-via-pipelines-023, eval = FALSE, tidy = FALSE}
    .train_invert = function(task) {
...
```{r target-transformations-via-pipelines-024, eval = FALSE, tidy = FALSE}
    .invert = function(prediction, predict_phase_state) {
...
```{r target-transformations-via-pipelines-025, echo = FALSE}
PipeOpTargetTrafoBoxCox = R6Class("PipeOpTargetTrafoBoxCox",
  inherit = PipeOpTargetTrafo,
  public = list(
    initialize = function(id = "targettrafoboxcox", param_vals = list()) {
...
```{r target-transformations-via-pipelines-027, fig.width=10, fig.height=10}
g_bc$plot()
```
