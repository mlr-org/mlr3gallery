...
The `r mlr_pkg("mlr3learners")`  package provides additional `r ref("Learner", "learners")`.
...
```{r target-transformations-via-pipelines-025}
g_bc$plot()
```

Finally we `$train()` and `$predict()` on the task (again, we wrap the `r ref("Graph")` in a `GraphLearner`):
