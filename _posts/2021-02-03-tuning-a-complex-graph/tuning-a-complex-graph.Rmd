...
```{r tuning-a-complex-graph-008, layout="l-body-outset", fig.width=6}
graph1$plot()
```
...
```{r tuning-a-complex-graph-028, graph2}
graph2$plot()
```
...
```{r tuning-a-complex-graph-034}
tune_ps2$trafo(x)
```

Tuning can be carried out analogously as done above:

```{r tuning-a-complex-graph-035, eval = FALSE}
...
```{r tuning-a-complex-graph-036, eval = FALSE}
