...
```{r 2020-07-27-bikesharing-demand-056, include = FALSE, eval = params$eval_all}
...
```{r 2020-07-27-bikesharing-demand-063, message=FALSE, results='hide', eval=FALSE}
search_space = ps(
  branch.selection = p_int(lower = 1L, upper = 3L),
  targetmutate.regr.kknn.targetinverter.regr.kknn.k = p_int(lower = 1L, upper = 50L, depends = branch.selection == 1),
  targetmutate.regr.kknn.targetinverter.regr.kknn.distance = p_int(lower = 1L, upper = 3L, depends = branch.selection == 1),
  targetmutate.regr.rpart.targetinverter.regr.rpart.minsplit = p_int(lower = 1L, upper = 30L, depends = branch.selection == 2),
  targetmutate.regr.rpart.targetinverter.regr.rpart.cp = p_dbl(lower = 0.001, upper = 0.1, depends = branch.selection == 2),
...
```{r 2020-07-27-bikesharing-demand-064, eval = TRUE}
multi_at = auto_tuner(
...
cols = c("regr.rmse", "branch.selection",
          "targetmutate.regr.kknn.targetinverter.regr.kknn.k",
          "targetmutate.regr.kknn.targetinverter.regr.kknn.distance",
          "targetmutate.regr.rpart.targetinverter.regr.rpart.minsplit",
          "targetmutate.regr.rpart.targetinverter.regr.rpart.cp",
          "importance.filter.nfeat", "importance.mtry")
multi_at_archiv = lapply(1:3, function(x) multi_at_rr$data$learner[[x]]$archive$data[, ..cols])
```

```{r 2020-07-27-bikesharing-demand-065, echo = FALSE, eval = TRUE}
saveRDS(multi_at_archiv, "data/multi_at_archiv.rda")
```


```{r 2020-07-27-bikesharing-demand-066, echo = FALSE}
multi_at_archiv = readRDS("data/multi_at_archiv.rda")
```
