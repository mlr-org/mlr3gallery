...
Note that [PipeOpDateFeatures](https://github.com/mlr-org/mlr3pipelines/blob/master/R/PipeOpDateFeatures.R) provides an alternative to engineer features.
...
mlr3 simplifies that process by Pipe Operators.
These `PipeOps` are computational steps that can be arranged Pipelines to manage dataflow in mlr3.
Using `PipeOpTargetTrafo` we can dynamicly transform our target later on.
...
Note, that this method does not lead to an unbiased GE, but rather a step to compare the learners to
each other.
