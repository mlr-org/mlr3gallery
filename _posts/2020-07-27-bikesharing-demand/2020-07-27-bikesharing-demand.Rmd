...
The minimal RMSE that the `r ref("AutoTuner")` could find is given for `p = 3` and `k = 12`.
...
`rpart` is CART learner whose hyperparameters are shown in Table 5.
...
autoplot(bmr, measure = msr("regr.rmse")) +
  theme_bw() +
  scale_x_discrete(labels = c("KNN", "Random Forest", "CART", "Random Forest Tuned"  ,  "KNN Tuned", "CART Tuned")) +
  labs(y = "RMSE") +
  coord_flip() +
  theme(axis.title = element_text(size = 9))
```
...
```{r 2020-07-27-bikesharing-demand-062, fig.cap="Pipeline Learner", fig.width = 5, fig.height = 4.3}
...
```{r 2020-07-27-bikesharing-demand-063, message = FALSE, results = 'hide', eval = TRUE}
search_space = ps(
  branch.selection = p_int(lower = 1L, upper = 3L),
  targetmutate.regr.kknn.targetinvert.regr.kknn.k = p_int(lower = 1L, upper = 50L, depends = branch.selection == 1),
  targetmutate.regr.kknn.targetinvert.regr.kknn.distance = p_int(lower = 1L, upper = 3L, depends = branch.selection == 1),
  targetmutate.regr.rpart.targetinvert.regr.rpart.minsplit = p_int(lower = 1L, upper = 30L, depends = branch.selection == 2),
  targetmutate.regr.rpart.targetinvert.regr.rpart.cp = p_dbl(lower = 0.001, upper = 0.1, depends = branch.selection == 2),
...
multi_at_archive = extract_inner_tuning_archives(multi_at_rr) %>%
  select(regr.rmse, branch.selection, targetmutate.regr.kknn.targetinvert.regr.kknn.k, 
    targetmutate.regr.kknn.targetinvert.regr.kknn.distance, targetmutate.regr.rpart.targetinvert.regr.rpart.minsplit,
    targetmutate.regr.rpart.targetinvert.regr.rpart.cp, importance.filter.nfeat, importance.mtry)
...
# saveRDS(data_total, "data_total.rda")
```

```{r 2020-07-27-bikesharing-demand-071, echo = FALSE, eval=!long_run}
# data_total = readRDS("data_total.rda")
```
