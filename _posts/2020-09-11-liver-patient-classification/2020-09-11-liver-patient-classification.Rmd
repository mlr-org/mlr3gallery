...
po_scale = po("scale")
po_scale$param_set$values$affect_columns = selector_name(c("age", "direct_bilirubin", "alkaline_phosphatase",
...
coldescr_scaled = cbind.data.frame(
  Variable = colnames(liver_df_log),
...
```{r task definition}
## Task definition
task_liver = TaskClassif$new(id = "task_liver", backend = liver_df_log, 
...
learners_bal = lapply(learners, function(x) GraphLearner$new(po_scale %>>% po_over %>>% x))
...
confusion <- lapply(setNames(1:14, names), function(x) ftable(bmr$resample_result(x)$prediction()$confusion))
```
