...
The data set we used for our project is the "Indian Liver Patient Dataset" which can be obtained from the UCI Machine Learning Repository; it was donated by three professors from India in 2012 @Dua2017.
...
```{r, echo=FALSE}
col_descr <- cbind.data.frame(Variable = colnames(liver_df),
                              Description = c("Age of the patient (all patients above 89 are labelled as 90",
                                              "Sex of the patient (1 = female, 0 = male)",
                                              "Total serum bilirubin (in mg/dL)",
                                              "Direct bilirubin level (in mg/dL)",
                                              "Serum alkaline phosphatase level (in U/L)",
                                              "Serum alanine transaminase level (in U/L)",
                                              "Serum aspartate transaminase level (in U/L)",
                                              "Total serum protein (in g/dL)",
...
As it turned out omitting or imputation techniques were not necessary, because we ended up not using the ratio due to high correlations with other features. 
...
GGally::ggcorr(liver_df_numeric, label = TRUE, label_color='white', hjust = .9)
```
...
Lastly, we standardized all metric features, as different ranges and units might weigh features. This is especially important for the k-NN model.
The following table shows the final dataset and the transformations we applied.  
**Note**: This should be regularly done by implementing a `r mlr3book::ref("PipeOpScale")` and other data dependent log transformations.
As these features won't be unscaled in prediction this might lead to misinterpretation of the data.
Furthermore updating the data set, or running the method on test sets requires to transform these new data by hand again.
Therefore we strongly recommend the usage of `mlr3pipelines` in such cases.


```{r, echo=FALSE}
coldescr_scaled <- cbind.data.frame(
  Variable = colnames(liver_df_scaled),
  Transformation = c("scaled", "none", "scaled and log-transformed", "scaled and log-transformed", "scaled and log-transformed", "scaled", "none")
)

kable(coldescr_scaled)
```
...
learners <- list(
  ...
  learner_lda = lrn("classif.lda", predict_type = "prob", predict_sets = c("train", "test")),
  learner_qda = lrn("classif.qda", predict_type = "prob", predict_sets = c("train", "test")),
...
The next step is to instantiate the AutoTuner from `mlr3tuning`.

```{r AutoTuner definition}
# AutoTuner for k-NN, CART and random forest
learners$learner_knn = AutoTuner$new(
...
learners$learner_rpart = AutoTuner$new(
...
learners$learner_rf = AutoTuner$new(
...
```{r oversampling and corresponding learners, results="hide"}
...
learners_bal = lapply(learners, function(x) GraphLearner$new(po_over %>>% x))
...
the solution is to wrap it minipages making sure to use a % and leave no spaces to join the two tables.

```{r two-column, results='asis', echo=FALSE, out.extra=''}
panderOptions("table.style", "grid")
cat("<h1>confusion matrices</h1>")
names <- sapply(1:14, function(x) bmr$resample_result(x)$learner$id)
cat("<table class='container'><tr>")
cat("<td>")
pander(lapply(setNames(1:7, names[1:7]), function(x) ftable(bmr$resample_result(x)$prediction()$confusion))) 
cat("</td>")
cat("<td>")
pander(lapply(setNames(8:14, names[8:14]), function(x) ftable(bmr$resample_result(x)$prediction()$confusion)))
cat("</td>")
cat("</tr></table>")
```
