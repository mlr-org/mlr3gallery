...
```{r 2020-03-30-stratification-blocking-001, include=FALSE}
...
In `r mlr_pkg("mlr3")`, each `r ref("Task")` has a slot [`$col_roles`](https://mlr3.mlr-org.com/reference/Task.html#active-bindings).
...
```{r 2020-03-30-stratification-blocking-004}
...
```{r 2020-03-30-stratification-blocking-005_roles_startum}
...
The following example is based on the [BreastCancer](https://www.rdocumentation.org/packages/mlbench/versions/2.1-1/topics/BreastCancer) data set from the `r cran_pkg("mlbench")` package:
...
```{r 2020-03-30-stratification-blocking-015}
...
```{r 2020-03-30-stratification-blocking-016_ blocked}
...
```{r 2020-03-30-stratification-blocking-017_blocked}
...
```{r 2020-03-30-stratification-blocking-019_folds1}
...
```{r 2020-03-30-stratification-blocking-020_folds2}
...
```{r 2020-03-30-stratification-blocking-021}
cv5 = rsmp("cv", folds = 5)
cv5$instantiate(task_bc)
cv5$instance
```
...
```{r 2020-03-30-stratification-blocking-022_folds3}
...
```{r 2020-03-30-stratification-blocking-023_folds4}
task_gc = tsk("german_credit")
...
```{r 2020-03-30-stratification-blocking-024_folds5}
...
This post shows how to control the resampling process when using `r mlr_pkg("mlr3")` in order to account for data specificities.
