...
The out of box random forest appears to already have good performance. Regarding the LDA, we do get a warning message, that some features are colinear. This strongly suggests to reduce the dimensionality of the feature space.
...
Finally, regarding the dependency of `classif.ranger.mtry` on `pca.rank.` and `ica.n.comp`, we define a trafo function. Here `x` will contain a named list of parameters sampled from the `param_set`. We store the `pca.rank.` or `ica.n.comp` value depending on which preprocessing step was sampled and we set `classif.ranger.mtry` to this number if it exceeded it.
...
We can now tune the parameters of our pipeline as defined in the search space with respect to a measure. Here, we will use the classification accuracy. As a resampling method we use `3-fold cross-validation`. We will use the `TerminatorNone` for terminating the tuning because we will apply a grid search.
...
We then perform a grid search using a resolution of 4 (this means that for each numeric hyperparameter, 4 equidistant values are taken, including the lower and upper bound):

FIXME: This takes way too long. Save as RDS file?

```{r}
tnr("grid_search", resolution = 4)$tune(tune1)
```

Now, we can inspect the results based on what was sampled by the tuner, ordered by the classification accuracy:

```{r}
tune1_results = tune1$archive(unnest = "tune_x")
tune1_results[order(classif.acc), ]
```

We achieve very good accuracy using ICA with 30 components and LDA with moment estimation.
...
pipeline1$param_set$values$pca.rank. = 30
...
Instead of using branches to split our pipeline with respect to the preprocessing and learner options, we can also use proxy pipelines.
`PipeOpProxy` aceepts a single `content` parameter that can contain any other `PipeOp` or `Graph`. This is extremely flexible in the sense that we do not have to specify our options during construction. However, the hyperparameters of the contained `PipeOp` or `Graph` are no longer directly contained in the `ParamSet` of the resulting `PipeOp`. Therefore, these must be specified when specifying the `content` and when tuning the pipeline, we do have to make use of a trafo function.

```{r}
pipeline2 = po("colapply", param_vals = list(applicator = function(x) log(x, base = 10))) %>>%
  po("scale") %>>%
  po("proxy", id = "proxy_preproc") %>>%
  po("proxy", id = "proxy_learner")
```
...
At first, this may look like a linear pipeline. However, as the `content` parameter of `PipeOpProxy` can be tuned, this will allow for a similar non-linear pipeline as when doing branching.

```{r}
pipeline2$param_set$ids()
```

We can tune the pipeline by using the same search space as before. However, here the trafo function is of central importance to actually set our options:

```{r}
tune_ps2 = tune_ps1$clone(deep = TRUE)
```

The trafo function does all the work, i.e., setting either `po("pca")` or `po("pca")` as the `proxy_preproc.content`, setting either `lrn("classif.lda")` or `lrn("classif.ranger")` as the `proxy_learner.content` as well as setting the hyperparameters of the respective preprocessing `PipeOps` and `Learners`. Also, we do handle the dependency of `classif.ranger.mtry` on `pca.rank.` and `ica.n.comp` here.

```{r}
tune_ps2$trafo = function(x, param_set) {
  proxy_preproc_options = list(pca = po("pca"), ica = po("ica"))
  proxy_learner_options = list(classif.lda = lrn("classif.lda"), classif.ranger = lrn("classif.ranger"))
  values_out = list(proxy_preproc.content = proxy_preproc_options[[x$branch_preproc.selection]],
    proxy_learner.content = proxy_learner_options[[x$branch_learner.selection]])
  if (x$branch_preproc.selection == "pca") {
    values_out$proxy_preproc.content$param_set$values$rank. = x$pca.rank.
    ndim = x$pca.rank.
  } else {
    values_out$proxy_preproc.content$param_set$values$n.comp = x$ica.n.comp
    ndim = x$ica.n.comp
  }
  if (x$branch_learner.selection == "classif.lda") {
    values_out$proxy_learner.content$param_set$values$method = x$classif.lda.method
  } else {
    values_out$proxy_learner.content$param_set$values$mtry = if (x$classif.ranger.mtry > ndim) ndim else x$classif.ranger.mtry
    values_out$proxy_learner.content$param_set$values$num.trees = x$classif.ranger.num.trees
  }
  values_out
}
```

Tuning then can be carried out as done above.
