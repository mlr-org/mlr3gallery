...
```{r}
tnr("grid_search", resolution = 4)$optimize(tune1)
```
...
Tuning then can be carried out analogously as done above (note that setting `unnest = "x_domain"` allows us to inspect the parameters as they were selected prior to the trafo function being applied):
...
tnr("grid_search", resolution = 4)$optimize(tune2)

tune2_results = tune2$archive$data(unnest = "x_domain")
