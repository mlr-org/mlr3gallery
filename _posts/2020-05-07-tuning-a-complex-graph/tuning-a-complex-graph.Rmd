...
We can now tune the parameters of our graph as defined in the search space with respect to a measure. Here, we will use the `classification accuracy`. As a resampling method we use `3-fold` `cross-validation`. We will use the `r ref("TerminatorNone", "TerminatorNone")` (i.e., no early termination) for terminating the tuning because we will apply a `grid search`:
