...
The out of box random forest appears to already have good performance on the training task. Regarding the LDA, we do get a warning message, that some features are colinear. This strongly suggests to reduce the dimensionality of the feature space.
...
Finally, regarding the dependency of `classif.ranger.mtry` on `pca.rank.` and `ica.n.comp`, we define a trafo function. Here `x` will contain a named list of parameters sampled/selected from the `param_set`. We store the `pca.rank.` or `ica.n.comp` value depending on which preprocessing step was sampled/selected and we set `classif.ranger.mtry` to this number if it exceeded it.
