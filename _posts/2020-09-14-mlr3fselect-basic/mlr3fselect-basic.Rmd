...
```{r mlr3fselect-basic-013, eval = FALSE}
...
```{r mlr3fselect-basic-028, eval = FALSE}
rr$score()
```

```{r mlr3fselect-basic-029, echo = FALSE}
...
```{r mlr3fselect-basic-030, eval = FALSE}
extract_inner_fselect_archives(rr)
```

```{r mlr3fselect-basic-031, echo = FALSE, layout="l-page"}
extract_inner_fselect_archives(rr) %>%
  kable(format = "html") %>%
  kable_styling(full_width = TRUE) %>%
...
```{r mlr3fselect-basic-033, eval = FALSE}
rr = fselect_nested(
  method = "random_search",
  task = tsk("iris"),
  learner = lrn("classif.log_reg"),
