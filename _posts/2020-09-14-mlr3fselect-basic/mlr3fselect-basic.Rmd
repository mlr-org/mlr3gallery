...
The `AutoFSelector` class inherits from the `Learner` class and starts an automatic feature selection when a task is passed to the `$train()` method.
 
```{r AutoFSelect}
fs_sequential = fs("sequential")
at_sequential = AutoFSelector$new(learner, resampling_inner, measure = measure, terminator, fselect = fs_sequential, store_fselect_instance = TRUE)

fs_random = fs("random_search")
at_random = AutoFSelector$new(learner, resampling_inner, measure = measure, terminator, fselect = fs_random, , store_fselect_instance = TRUE)
```

We combine each `AutoFSelector` learner with the task and resampling to a benchmark grid design.
