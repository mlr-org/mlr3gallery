...
date: 04-27-2020
description: |
  This tutorial explains how create and tune a multilevel stacking model using the mlr3pipelines package.
output:
...
1. **Level 0:** Based on the input data, we train three learners (`rpart`, `glmnet` and `lda`) on a sparser feature space obtained using different feature filter from [mlr3filters](https://mlr3filters.mlr-org.com/) to obtain slightly decorrelated predictions.
...
For the stacking example, we use the sonar task:
...
The out-of-bag predictions of the level 1 learner are attached to the input data from level 1 and a final ranger learner will be trained:  
