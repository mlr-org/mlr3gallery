...
We will evaluate all algorithms using 10-fold cross-validation.
...
In fact, if we zoom in to `(5, 30)` $\times$ `(0.2, 0.3)` and do decrease smoothing we see that different kernels have their optimum at different values of `k`:
