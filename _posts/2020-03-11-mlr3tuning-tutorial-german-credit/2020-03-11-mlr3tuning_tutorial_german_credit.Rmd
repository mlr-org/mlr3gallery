...
Also, because tuning often takes a long time, we want to make more efficient use of our multi-core CPUs.
...
As a next step, we define a `r ref("TuningInstance")` that represents the problem we are trying to optimize.
...
The result are returned by `$optimize()` together with its performance and 
can be also found in the `$result` slot.
...
We may find ttwoo problems when doing so:
...
Like before, we can review the `archive`. It includes the points before and after the transformation.
...
The parameters used by the learner (points after the transformation) are stored in
in the `x_domain` column as lists. By using `unnest = x_domain`, the lists elements are expanded 
to seperate columns.
...
grid_auto = AutoTuner$new(
  learner = knn,
  resampling = rsmp("holdout"), # we can NOT use fixed resampling here
  measure = msr("classif.ce"),
  search_space = search_space,
  terminator = trm("none"),
...
```{r, eval = FALSE}
rr = resample(task, grid_auto, cv10_instance)
```
...
We use the `r mlr_pkg("paradox")` package in this use case.
...
ParamFct$new("parameter_id", levels = c("value1", "value2", "value3"))
# - integer (from 'lower' to 'upper' bound)
ParamInt$new("parameter_id", lower = 0, upper = 10)
# - numeric (from 'lower' to 'upper' bound)
# - named after the storage type, "double precision floating point"
ParamDbl$new("parameter_id", lower = 0, upper = 10)

# also possible: "untyped", but we can not tune with this without a transformation function!
ParamUty$new("parameter_id")
```

So an example parameter set with one logical parameter `"flag"` and one integer parameter `"count"` looks like this:
