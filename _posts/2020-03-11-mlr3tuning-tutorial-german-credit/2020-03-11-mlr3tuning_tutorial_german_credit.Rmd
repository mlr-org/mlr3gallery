...
We evaluate all hyperparameter configurations using 10-fold cross-validation. We use a *fixed* train-test split, i.e. the same splits for each evaluation. Otherwise, some evaluation could get unusually "hard" splits, which would make comparisons unfair.
...
- Tuning works by calling `$optimize()`. Note that it *modifies* our "tuning instance"--the result can be found in the `instance_grid` object.
...
- The result are returned by `$optimize()` and can be also found in the `$result` slot.

```{r}
instance_grid$result
```

- We can look at the archive to get all evaluated hyperparameter configurations.

```{r}
instance_grid$archive$data()
```

- We use `ggplot` to plot the performance depending on the sampled `k` and `distance`.
