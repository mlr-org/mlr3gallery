...
Before that, though, we are interested in the parameter set on which we could actually tune.

```{r}
knn$param_set
```

We first tune the `k` parameter (i.e. the number of neighbours to be accounted for).
...
We do so by expandin the "params" (the parameters that the `Learner` actually saw).
