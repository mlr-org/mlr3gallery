...
The result is returned by `$optimize()` together with its performance. It can 
be also accessed with the  `$result` slot.
...
in the `x_domain` column as lists. By using `unnest = x_domain`, the list elements are expanded 
to separate columns.
