...
```{r 2020-03-11-mlr3tuning-tutorial-german-credit-004, include=FALSE}
...
```{r 2020-03-11-mlr3tuning-tutorial-german-credit-006, warning=FALSE}
...
```{r 2020-03-11-mlr3tuning-tutorial-german-credit-009}
knn$param_set
```
...
```{r 2020-03-11-mlr3tuning-tutorial-german-credit-014, eval = params$eval_all}
tuner_grid$optimize(instance_grid)
```

```{r 2020-03-11-mlr3tuning-tutorial-german-credit-015, echo = FALSE, eval = params$eval_all}
saveRDS(instance_grid, "data/instance_grid.rda")
```

```{r 2020-03-11-mlr3tuning-tutorial-german-credit-016, echo = FALSE}
...
```{r 2020-03-11-mlr3tuning-tutorial-german-credit-017}
instance_grid$result
```
...
```{r 2020-03-11-mlr3tuning-tutorial-german-credit-022, eval = params$eval_all}
tuner_random$optimize(instance_random)
```

```{r 2020-03-11-mlr3tuning-tutorial-german-credit-023, echo = FALSE, eval = params$eval_all}
saveRDS(instance_random, "data/instance_random.rda")
```

```{r 2020-03-11-mlr3tuning-tutorial-german-credit-024, echo = FALSE}
...
The `mlr3tuning` package provides an `r ref("AutoTuner")` that acts like our tuning method but is actually a `r ref("Learner")`.
...
The `AutoTuner` behaves just like a regular `r ref("Learner")`.
...
```{r 2020-03-11-mlr3tuning-tutorial-german-credit-030, eval = params$eval_all}
rr = resample(task, grid_auto, cv10_instance, store_models = TRUE)
```

```{r 2020-03-11-mlr3tuning-tutorial-german-credit-031, echo = FALSE, eval = params$eval_all}
...
```{r 2020-03-11-mlr3tuning-tutorial-german-credit-033}
rr$aggregate()
```
...
```{r 2020-03-11-mlr3tuning-tutorial-german-credit-034}
rr$learners[[1]]
...
```{r 2020-03-11-mlr3tuning-tutorial-german-credit-035, echo=FALSE, eval=FALSE}
set.seed(2409)
instance_random = TuningInstanceSingleCrit$new(
  task = task,
  learner = knn,
...
```{r 2020-03-11-mlr3tuning-tutorial-german-credit-036, echo=FALSE}
...
```{r 2020-03-11-mlr3tuning-tutorial-german-credit-038}
...
```{r 2020-03-11-mlr3tuning-tutorial-german-credit-039, warning=FALSE}
ggplot(perfdata, aes(x = x_domain_k, y = classif.ce, color = kernel,
...
```{r 2020-03-11-mlr3tuning-tutorial-german-credit-040, warning=FALSE}
