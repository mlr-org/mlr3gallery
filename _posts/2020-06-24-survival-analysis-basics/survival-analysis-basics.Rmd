...
rsmp_cv4 = rsmp("cv", folds = 4)
msr_graf = msr("surv.graf")
tnr_rs   = tnr("random_search")
trm_e10  = trm("evals", n_evals = 2)# increase for better exploration of search space
```
...
params_glmnet = ParamSet$new(
  list(
    ParamDbl$new("surv.glmnet.alpha", lower = 0L, upper = 1L),
    ParamDbl$new("surv.glmnet.lambda", lower = 0.0001, upper = 1)
  )
)
...
lrn_glmnet_auto  = AutoTuner$new(
  learner      = ppl("distrcompositor", lrn("surv.glmnet"), graph_learner = TRUE),
...
graph_glmnet = GraphLearner$new(
  id = "GLMnet",
  po("encode") %>>% lrn_glmnet_auto)
# auto tuner ranger
lrn_ranger  = AutoTuner$new(
  learner      = lrn("surv.ranger", num.trees = 200L),
...
design <- benchmark_grid(
  tasks = churn_tsk,
  learners = list(lrn_km, lrn_cph, graph_glmnet, lrn_ranger),
...
```{r, eval = FALSE}
bmr$aggregate(msr("surv.graf"))
bmr$aggregate(msr("surv.cindex"))
# autoplot(bmr, measure = msr("surv.graf"))
```
