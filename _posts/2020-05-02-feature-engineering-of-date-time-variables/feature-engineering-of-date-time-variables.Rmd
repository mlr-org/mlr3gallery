...
task = as_task_regr(bikes, target = "cnt")

train_set = sample(seq_len(task$nrow), size = 0.7 * task$nrow)
...
task_train_ranger$select(setdiff(task$feature_names, 
  c("instant", "dteday", "date", "casual", "registered")))
```

We can then use `r ref("resample()")` with 3-fold cross-validation:

```{r feature-engineering-of-date-time-variables-010, results = 'hide'}
rr_ranger = resample(task_train_ranger, learner = learner_ranger, resampling = cv3)

rr_ranger$score(msr("regr.mse"))
```

```{r mlr3fselect-basic-02729, echo = FALSE}
...
task_train_ex = task_train$clone()
task_train_ex$select(setdiff(task$feature_names,
  c("instant", "dteday", "yr", "mnth", "hr", "weekday", "casual", "registered")))

pipeop_date$train(list(task_train_ex))
```
...
```{r feature-engineering-of-date-time-variables-014, fig.width=8, fig.height=10}
...
Using `r ref("resample()")` with 3-fold cross-validation on the training task yields:

```{r feature-engineering-of-date-time-variables-015, results = 'hide'}
task_train_graph_learner = task_train$clone()
task_train_graph_learner$select(setdiff(task$feature_names,
  c("instant", "dteday", "yr", "mnth", "hr", "weekday", "casual", "registered")))

rr_graph_learner = resample(task_train_graph_learner, learner = graph_learner, resampling = cv3)
...
Finally, we fit our `r ref("GraphLearner")` on the complete training task and predict on the test task:

```{r feature-engineering-of-date-time-variables-017}
task_train$select(setdiff(task$feature_names,
  c("instant", "dteday", "yr", "mnth", "hr", "weekday", "casual", "registered")))
...
task_test$select(setdiff(task$feature_names,
  c("instant", "dteday", "yr", "mnth", "hr", "weekday", "casual", "registered")))

prediction = graph_learner$predict(task_test)
