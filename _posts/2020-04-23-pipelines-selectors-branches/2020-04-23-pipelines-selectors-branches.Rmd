...
Using the `affect_columns` argument of a `r ref("PipeOp")` to define the variables on which a `r ref("PipeOp")` will operate with an appropriate [`selector`](https://mlr3pipelines.mlr-org.com/reference/Selector.html) function:
...
```{r 2020-04-23-pipelines-selectors-branches-007, fig.height = 6, fig.width = 6}
...
```{r 2020-04-23-pipelines-selectors-branches-009, fig.height = 6, fig.width = 6}
...
We can use the [`select`](https://mlr3pipelines.mlr-org.com/reference/mlr_pipeops_select.html) `r ref("PipeOp")` to select the appropriate features and then apply the desired impute `r ref("PipeOp")` on them:

```{r 2020-04-23-pipelines-selectors-branches-010, fig.height = 6, fig.width = 6}
hist_imp2 = po("select",
...
```{r 2020-04-23-pipelines-selectors-branches-013, fig.height = 7, fig.width = 7}
...
```{r 2020-04-23-pipelines-selectors-branches-014, fig.height = 7, fig.width = 7}
...
```{r 2020-04-23-pipelines-selectors-branches-017}
...
```{r 2020-04-23-pipelines-selectors-branches-018}
instance$result
```

## Conclusion

This post shows ways on how to specify features on which preprocessing steps are to be performed. In addition it shows how to create alternative paths in the learner graph. The preprocessing steps that can be used are not limited to imputation. Check the list of available [`r ref("PipeOp")`s](https://mlr3pipelines.mlr-org.com/reference/index.html).
