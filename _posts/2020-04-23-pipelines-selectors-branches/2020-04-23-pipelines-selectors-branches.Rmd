...
- [mlr3pipelines tutorial - german credit](https://mlr3gallery.mlr-org.com/basics_pipelines_german_credit/)
- [Impute missing variables](https://mlr3gallery.mlr-org.com/impute-missing-variables/) .  
...
A common approach in such situations is to impute the missing values and to add a missing indicator column as explained in the [Impute missing variables](https://mlr3gallery.mlr-org.com/impute-missing-variables/) post.
...
When using `PipeOps` constructed in this way, they will perform the specified preprocessing step on the appropriate columns and pass the result to the subsequent steps:  
...
In each preprocessing step, the final result is passed to next step. 
...
Obviously, this step can not be apply to the already imputed data as there are no missing values.
...
Note that when there is one input channel, it is automatically copied as many times as needed for the downstream `PipeOps`. 
...
The best performance in this short tuned experiment was achieved with the more complicated branch.  
