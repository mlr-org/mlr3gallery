...
To illustrate this, we take a learner with a integer-valued hyperparameter.
...
The archive shows us that for all real-valued `k` proposed by `GenSA`, 
an integer-valued `k` in the learner hyperparameter space (`x_domain_k`) was created. 
...
Note that the tuner is not aware of the transformation. This could lead to problems
since the tuner proposes different real values but they might be all rounded to
