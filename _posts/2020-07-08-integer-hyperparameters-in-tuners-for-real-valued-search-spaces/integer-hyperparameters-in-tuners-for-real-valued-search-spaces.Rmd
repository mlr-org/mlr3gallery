...
However, it is possible to round the real values proposed by a `Tuner` to integers before passing them to the learner in the evaluation. 
We show how to apply a parameter transformation to a `paradox::ParamSet` and use this set in the tuning process.
...
The `param_classes` field of `TunerGenSA` states that the tuner only supports real-valued (`ParamDbl`) hyperparameter tuning.
...
```{r}
search_space = ps(
  k = p_dbl(lower = 3, upper = 8, trafo = as.integer)
)
```
...
Internally, `TunerGenSA` was given the parameter types of the search space and therefore suggested real numbers for `k`.
...
This could lead to problems since the tuner proposes different real values but they might be all rounded to the same integer value.

We successfully tuned a integer-valued hyperparameter with `TunerGenSA` which is only suitable for an real-valued search space.
This technique is not limited to tuning problems.
`Optimizer` in [`bbotk`](https://bbotk.mlr-org.com) can be also used in the same way to produce points with integer parameters.
