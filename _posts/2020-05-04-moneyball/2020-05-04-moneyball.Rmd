...
```{r 2020-05-04-moneyball-003, eval = FALSE}
oml_id = 41021
...
This special `r ref("Learner")` can be put into a graph together with the `imputer` via the `new` method of the `GraphLearner` class.
...
```{r 2020-05-04-moneyball-015, size = "tiny"}
...
Subsequently, we add the `PipeOpSelect` `r ref("PipeOp")` to the pipeline.

```{r 2020-05-04-moneyball-021}
...
We also learned how to use some `r ref("PipeOp")`s from the `mlr3pipelines` package.
