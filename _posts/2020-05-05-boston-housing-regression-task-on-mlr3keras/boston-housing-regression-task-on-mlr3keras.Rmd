...
This use case shows how to use `r gh_pkg("mlr-org/mlr3keras")` on simple
...
is translated to `r gh_pkg("mlr-org/mlr3keras")` respectively `r gh_pkg("mlr-org/mlr3keras")` syntax.
...
As `r gh_pkg("mlr-org/mlr3keras")` is still under heavy development, this place might be 
...
training more difficult, and it makes the resulting model more dependent on the
choice of units used in the input.
We create a `r ref("mlr_pipeops_scale", "PipeOpScale")` that is going to be
connected to the learner before training.
...
A final denselayer returns a single continuous estimation value for the
observations target.
...
A `callback` is a set of functions, bundled in a list that can be used to
...
model by default.
It stores the training history in a `history` slot of its' concerning model
object.
...
Therefore we create a `LambdaCallback` that applied at each epochs end.
...
Before we train the learner we divide observations randomly into train and test
set.
The training set contains 404 observations and the test set 102.
Now we can train our learner, using the `training_id`.
Reminder: As we set it in `callback`, each `epoch` is represented by a dot.
...
In this case we can implement our Early stopping callback.
`early_stop` will stop the model if it shows no improvement in `val_loss` within
20 `epochs`.

### Train with early stopping

This is exactly the same keras model, using early stopping `callback`.
...
2. Use `reset_random_seeds` or respectively build you own set reset function
(see below).
...
We introduced this function because in some incidences you might run into an
...
In these cases it makes sense to create your own function to set seeds for
`keras`.
...
GPU computations and CPU palatalization can be another source of  
non-reproducible, since both can result in non-deterministic execution
patterns.
...
`use_session_with_seed()` disables both GPU and CPU by default. For more detailed
