...
The R packages `mlr3`, `r mlr_pkg("mlr3pipelines")`  and `mlr3tuning` will be used.
...
* Define imbalance correction pipeline [`r ref("Graph")`](https://mlr3pipelines.mlr-org.com/reference/Graph.html)s (undersampling, oversampling and SMOTE) with `r mlr_pkg("mlr3pipelines")` 
* Autotune the `r ref("Graph")` together with a learner using `mlr3tuning`
* Benchmark the autotuned `r ref("Graph")` and visualize the results using `mlr3viz`
...
In `r mlr_pkg("mlr3pipelines")` , there is a [`classbalancing`](https://mlr3pipelines.mlr-org.com/reference/mlr_pipeops_classbalancing.html) and a [`smote`](https://mlr3pipelines.mlr-org.com/reference/mlr_pipeops_smote.html) pipe operator that can be combined with any learner.
...
We apply the `r ref("PipeOp")`s/`r ref("Graph")` to the current `task` with specific hyperparameter values to see how the class balance changes:
...
We combine the `r ref("PipeOp")`s/`r ref("Graph")` with a learner (here `ranger`) to make each pipeline graph behave like a learner:
...
```{r 2020-03-30-imbalanced-data-014, eval = params$eval_all}
set.seed(1)
bmr = benchmark(design, store_models = TRUE)
```

```{r 2020-03-30-imbalanced-data-015, echo = FALSE, eval = params$eval_all}
saveRDS(bmr, "data/bmr.rda")
```

```{r 2020-03-30-imbalanced-data-016, echo = FALSE}
bmr = readRDS("data/bmr.rda")
```

### Visualize benchmark results

```{r 2020-03-30-imbalanced-data-017}
