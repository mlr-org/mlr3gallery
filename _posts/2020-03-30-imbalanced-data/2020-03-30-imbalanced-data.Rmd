...
Please note that the `optdigits` dataset is also included in the `mlr3data` package where you can get the preprocessed (integers properly encoded as such, etc.) task via:

```{r}
task = get_optdigits_task()
```
...
```{r}
library(ggplot2)
stune_path = bmr$data$learners()[[2]][[1]]$model$tuning_instance$archive$data()
...
utune_path = bmr$data$learners()[[2]][[2]]$model$tuning_instance$archive$data()
