...
```{r}
library(ggplot2)
utune_path = bmr$data$state[[1]]$model$tuning_instance$archive$data()

utune_gg = ggplot(utune_path, aes(x = undersample.ratio, y = classif.fbeta)) +
  geom_point(size = 3) +
  geom_line() + ylim(0.9, 1)

otune_path = bmr$data$state[[2]]$model$tuning_instance$archive$data()
otune_gg = ggplot(otune_path, aes(x = oversample.ratio, y = classif.fbeta)) +
  geom_point(size = 3) +
  geom_line() + ylim(0.9, 1)

stune_path = bmr$data$state[[3]]$model$tuning_instance$archive$data()
