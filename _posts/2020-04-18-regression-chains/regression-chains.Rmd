...
For more details, see e.g. [Spyromitros-Xioufis, Tsoumakas, Groves & Vlahavas (2016)](https://doi.org/10.1007/s10994-016-5546-z).
...
This type of `Task` can be created via the [mlr3multioutput](https://github.com/mlr-org/mlr3multioutput/) (currently under development) in the future.
...
Writing pipelines with such steps allows for great flexibility in modelling more complicated scenarios such as the ones described below.
...
As `r ref("Learner", "Learners")` we will use simple linear regression models, `r ref("mlr_learners_regr.lm")`. Our pipeline building the regression chain then has to do the following:
...
Again, we can train to see how the output and `$state` look like, but now using the output of `step1`:
...
In the final third step we define $y_{3}$ as the new target (again, `PipeOpUpdateTarget` drops the original target from the feature set, here $y_{2}$):
...
By wrapping our `Graph` in a `r ref("GraphLearner")`, we can perform `3-fold` `r ref("ResamplingCV", "cross-validation")` and get an estimated average of the root-mean-square error (of course, in a real world setting splitting the data in a training and test set should have been done):
...
task_predict = TaskRegr$new("multiregression_predict", backend = dat_predict, target = "y1")
graph$train(task)
graph$predict(task_predict)
```

Note that we have to initialize the `Task` with $y_{1}$ as the target but the pipeline will automatically predict $y_{3}$ in the final step as our final target, which was our ultimate goal.
