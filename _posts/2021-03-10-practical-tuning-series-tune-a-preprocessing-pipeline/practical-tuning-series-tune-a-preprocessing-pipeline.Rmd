...
```{r practical-tuning-series-tune-a-preprocessing-pipeline-002, include=FALSE, eval = params$eval_all}
...
```{r practical-tuning-series-tune-a-preprocessing-pipeline-003, message = FALSE}
library(mlr3verse)
```
...
We wrap the `r ref("Graph")` into `r ref("GraphLearner")` with `r ref("as_learner()")` which allows us to use the graph like a normal learner.

```{r practical-tuning-series-tune-a-preprocessing-pipeline-015}
...
```{r practical-tuning-series-tune-a-preprocessing-pipeline-016, results = 'hide'}
...
```{r practical-tuning-series-tune-a-preprocessing-pipeline-019}
...
```{r practical-tuning-series-tune-a-preprocessing-pipeline-020, results = 'hide'}
instance = tune(
  method = "grid_search",
  task = task,
...
```{r practical-tuning-series-tune-a-preprocessing-pipeline-023}
graph_2 = po("encode") %>>%
  po("imputeoor") %>>%
  learner
...
```{r practical-tuning-series-tune-a-preprocessing-pipeline-024, eval = params$eval_all}
...
```{r practical-tuning-series-tune-a-preprocessing-pipeline-025, echo = FALSE, eval = params$eval_all}
saveRDS(bmr, "data/bmr_1.rda")
```

```{r practical-tuning-series-tune-a-preprocessing-pipeline-026, echo = FALSE}
...
```{r practical-tuning-series-tune-a-preprocessing-pipeline-027}
bmr$aggregate()
autoplot(bmr)
```
...
```{r practical-tuning-series-tune-a-preprocessing-pipeline-028, eval = FALSE}
at_2$train(task)
```
