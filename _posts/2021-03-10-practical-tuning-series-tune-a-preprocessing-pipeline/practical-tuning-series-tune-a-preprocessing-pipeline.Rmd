...
```{r practical-tuning-series-tune-a-preprocessing-pipeline-013, fig.width=10, fig.height=10}
graph = po("encode") %>>%
  po("imputeoor") %>>%
  learner
plot(graph)
...
```{r practical-tuning-series-tune-a-preprocessing-pipeline-023, eval = params$eval_all}
...
```{r practical-tuning-series-tune-a-preprocessing-pipeline-024, echo = FALSE, eval = params$eval_all}
saveRDS(bmr, "data/bmr_1.rda")
```

```{r practical-tuning-series-tune-a-preprocessing-pipeline-025, echo = FALSE}
...
```{r practical-tuning-series-tune-a-preprocessing-pipeline-026}
bmr$aggregate()
autoplot(bmr)
```
...
```{r practical-tuning-series-build-an-automated-machine-learning-system-028, eval = FALSE}
...
```{r practical-tuning-series-tune-a-preprocessing-pipeline-027, eval = FALSE}
at_2$train(task)
```
