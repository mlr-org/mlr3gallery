...
We can load it using the `r ref("tsk()")` function.
...
We also recommend the packages `r cran_pkg("skimr")` (`r ref("skimr::skim()")`) and `r cran_pkg("DataExplorer")` (`r ref("DataExplorer::plot_bar()")`, `r ref("DataExplorer::plot_histogram()")`, `r ref("DataExplorer::plot_boxplot()")`) packages as they create very well readable and understandable overviews.
...
```{r, echo=FALSE}
german_short = mlr3misc::map_dtc(german, function(x) {
...
```{r}
task = TaskClassif$new("GermanCredit", backend = german, target = "credit_risk")
```

Note that this task is an example task which ships with the `mlr3` package.
...
We compare `r ref("mlr_measures_classif.ce", text = "misclassification rate")` and `r ref("mlr_measures_classif.auc", text = "AUC")`.
