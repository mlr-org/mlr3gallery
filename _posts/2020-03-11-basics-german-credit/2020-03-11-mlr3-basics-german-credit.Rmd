...
-->
This includes packages from the `mlr3` family, as well as other packages for data handling, cleaning and visualization which we are going to use (`r cran_pkg("data.table")`, `r cran_pkg("ggplot2")`, `r cran_pkg("rchallenge")`, and `r cran_pkg("skimr")`).
...
```{r 2020-03-11-mlr3-basics-german-credit-005, R.options=list(width = 120)}
skimr::skim(german)
```
...
```{r 2020-03-11-mlr3-basics-german-credit-006, echo=TRUE}
#german_short = german
...
```{r 2020-03-11-mlr3-basics-german-credit-007, eval=FALSE}
#DataExplorer::plot_bar(german_short, nrow = 6, ncol = 3)
```

```{r 2020-03-11-mlr3-basics-german-credit-008, echo=FALSE, out.width="100%", fig.height=18}
#DataExplorer::plot_bar(german_short, nrow = 6, ncol = 3)
```

```{r 2020-03-11-mlr3-basics-german-credit-009, out.width="100%", fig.height=3}
#DataExplorer::plot_histogram(german_short, nrow = 1, ncol = 3)
```

```{r 2020-03-11-mlr3-basics-german-credit-010, out.width="100%", fig.height=3}
...
As `mlr3` usually only wraps learners from packages, it is even easy to create a formal `r ref("Learner")` by yourself.
...
All available `r ref("Learner")`s (i.e. all which you have installed from `mlr3`, `mlr3learners`, `mlr3extralearners`, or self-written ones) are registered in the dictionary `r ref("mlr_learners")`:
...
```{r 2020-03-11-mlr3-basics-german-credit-013}
library("mlr3learners")
...
```{r 2020-03-11-mlr3-basics-german-credit-014}
...
```{r 2020-03-11-mlr3-basics-german-credit-016}
head(train_set)
```
...
```{r 2020-03-11-mlr3-basics-german-credit-018, eval=TRUE}
learner_logreg$model
```

The stored object is a normal `glm` object and all its `S3` methods work as expected:

```{r 2020-03-11-mlr3-basics-german-credit-019, eval=TRUE}
...
```{r 2020-03-11-mlr3-basics-german-credit-021, eval=TRUE}
...
```{r 2020-03-11-mlr3-basics-german-credit-032}
...
```{r 2020-03-11-mlr3-basics-german-credit-034}
# false positive rate
...
```{r 2020-03-11-mlr3-basics-german-credit-035}
mlr_resamplings
mlr_measures
```
...
```{r 2020-03-11-mlr3-basics-german-credit-037, eval=FALSE}
...
```{r 2020-03-11-mlr3-basics-german-credit-039, eval=TRUE}
...
```{r 2020-03-11-mlr3-basics-german-credit-040, eval=2}
?ranger::ranger
...
```{r 2020-03-11-mlr3-basics-german-credit-043}
bmr = benchmark(bm_design)
print(bmr)
```
...
```{r 2020-03-11-mlr3-basics-german-credit-044}
measures = msrs(c("classif.ce", "classif.auc"))
performances = bmr$aggregate(measures)
...
```{r 2020-03-11-mlr3-basics-german-credit-045}
mlr_tasks
...
```{r 2020-03-11-mlr3-basics-german-credit-046}
names(pred_rf)
```
...
```{r 2020-03-11-mlr3-basics-german-credit-047}
class(pred_rf)
```
