...
Suppose `mlr3` would not contain a predefined `Task` for the German credit dataset.
We could then initialize a classification task using `TaskClassif$new()`.
...
All available `Learner`s (i.e. all which you have installed from `mlr3`, `mlr3learners`, the [mlr3learners github organization](https://github.com/mlr3learners/), or self-written ones) can be listed in the following dictionary:
...
Use `?mlr_learners_xxx` to open the help page of a learner named `xxx`, or the [rendered overview page](https://mlr3learners.mlr-org.com/reference/index.html).
...
We compare `r ref("mlr_measures_ce", text = "misclassification rate")` and `r ref("mlr_measures_auc", text = "AUC")`.
