...
```{r 2020-03-11-mlr3-basics-german-credit-001, include=FALSE}
...
This includes the `r mlr_pkg("mlr3verse")` package, as well as other packages for data handling, cleaning and visualization which we are going to use (`r cran_pkg("data.table")`, `r cran_pkg("ggplot2")`, `r cran_pkg("rchallenge")`, and `r cran_pkg("skimr")`).

Then, load the main packages we are going to use:

```{r 2020-03-11-mlr3-basics-german-credit-002, message=FALSE, warning=FALSE}
library("mlr3verse")
...
```{r 2020-03-11-mlr3-basics-german-credit-003, message=FALSE}
...
```{r 2020-03-11-mlr3-basics-german-credit-004}
dim(german)
str(german)
```
...
We also recommend the `r cran_pkg("skimr")` package as it creates very well readable and understandable overviews:

```{r 2020-03-11-mlr3-basics-german-credit-005}
skimr::skim(german)
```
...
Considering how we are going to tackle the problem of classifying the credit risk relates closely to what `r mlr_pkg("mlr3")` entities we will use.
...
Within `r mlr_pkg("mlr3")`, to distinguish between these problems, we define `r ref("Task", text = "Tasks")`.
...
```{r 2020-03-11-mlr3-basics-german-credit-006}
task = as_task_classif(german, id = "GermanCredit", target = "credit_risk")
```

Note that the German credit data is also given as an example task which ships with the `r mlr_pkg("mlr3")` package.
...
As `r mlr_pkg("mlr3")` usually only wraps learners from packages, it is even easy to create a formal `r ref("Learner")` by yourself.
...
If you happen to write your own `r ref("Learner")` in `r mlr_pkg("mlr3")`, we would be happy if you share it with the `r mlr_pkg("mlr3")` community.

All available `r ref("Learner")`s (i.e. all which you have installed from `r mlr_pkg("mlr3")`, `mlr3learners`, `mlr3extralearners`, or self-written ones) are registered in the dictionary `r ref("mlr_learners")`:

```{r 2020-03-11-mlr3-basics-german-credit-007}
mlr_learners
```
...
```{r 2020-03-11-mlr3-basics-german-credit-008}
library("mlr3learners")
...
```{r 2020-03-11-mlr3-basics-german-credit-009}
...
```{r 2020-03-11-mlr3-basics-german-credit-010}
...
```{r 2020-03-11-mlr3-basics-german-credit-011}
head(train_set)
```

In `r mlr_pkg("mlr3")` the training with a subset of the data can be declared by the additional argument `row_ids = train_set`:

```{r 2020-03-11-mlr3-basics-german-credit-012}
learner_logreg$train(task, row_ids = train_set)
```

The fitted model can be accessed via:

```{r 2020-03-11-mlr3-basics-german-credit-013, eval=TRUE}
learner_logreg$model
```

The stored object is a normal `glm` object and all its `S3` methods work as expected:

```{r 2020-03-11-mlr3-basics-german-credit-014, eval=TRUE}
...
```{r 2020-03-11-mlr3-basics-german-credit-015}
...
```{r 2020-03-11-mlr3-basics-german-credit-016, eval=TRUE}
...
```{r 2020-03-11-mlr3-basics-german-credit-017}
...
```{r 2020-03-11-mlr3-basics-german-credit-018}
prediction_logreg = learner_logreg$predict(task, row_ids = test_set)
prediction_rf = learner_rf$predict(task, row_ids = test_set)
```

```{r 2020-03-11-mlr3-basics-german-credit-019}
prediction_logreg
```

```{r 2020-03-11-mlr3-basics-german-credit-020}
prediction_rf
```
...
```{r 2020-03-11-mlr3-basics-german-credit-021}
...
```{r 2020-03-11-mlr3-basics-german-credit-022}
learner_logreg$predict_type = "prob"
```

```{r 2020-03-11-mlr3-basics-german-credit-023}
...
Within `r mlr_pkg("mlr3")`, we need to specify the resampling strategy using the `r ref("rsmp()")` function:

```{r 2020-03-11-mlr3-basics-german-credit-024}
...
```{r 2020-03-11-mlr3-basics-german-credit-025}
...
```{r 2020-03-11-mlr3-basics-german-credit-026}
res$aggregate()
```
...
```{r 2020-03-11-mlr3-basics-german-credit-027}
...
```{r 2020-03-11-mlr3-basics-german-credit-028}
...
```{r 2020-03-11-mlr3-basics-german-credit-029}
# false positive rate
...
```{r 2020-03-11-mlr3-basics-german-credit-030}
mlr_resamplings
mlr_measures
```
...
```{r 2020-03-11-mlr3-basics-german-credit-031}
...
For parallelization, `r mlr_pkg("mlr3")` relies on the `r cran_pkg("future")` package:

```{r 2020-03-11-mlr3-basics-german-credit-032, eval=FALSE}
...
```{r 2020-03-11-mlr3-basics-german-credit-033}
...
The previously shown techniques build the backbone of a `r mlr_pkg("mlr3")`-featured machine learning workflow.
...
```{r 2020-03-11-mlr3-basics-german-credit-034, eval=TRUE}
...
```{r 2020-03-11-mlr3-basics-german-credit-035, eval=2}
?ranger::ranger
...
```{r 2020-03-11-mlr3-basics-german-credit-036}
...
```{r 2020-03-11-mlr3-basics-german-credit-037}
...
```{r 2020-03-11-mlr3-basics-german-credit-038}
bmr = benchmark(grid)
print(bmr)
```
...
```{r 2020-03-11-mlr3-basics-german-credit-039}
measures = msrs(c("classif.ce", "classif.auc"))
performances = bmr$aggregate(measures)
...
This tutorial was a detailed introduction to machine learning workflows within `r mlr_pkg("mlr3")`.
...
The next parts of the tutorial will go more into depth into additional `r mlr_pkg("mlr3")` topics:
...
```{r 2020-03-11-mlr3-basics-german-credit-040}
mlr_tasks
...
- What are the possible slots and functions of an object?
  - Writing `prediction_rf$`, and pressing \<TAB\> should work.
    Otherwise:

```{r 2020-03-11-mlr3-basics-german-credit-041}
names(prediction_rf)
```
...
```{r 2020-03-11-mlr3-basics-german-credit-042}
class(prediction_rf)
```
