...
You can uncomment the following line if you are running this locally (i.e. **not** on RStudio cloud):
...
We use the `r mlr_pkg("mlr3pipelines")` package that enables us to chain `r ref("PipeOp", "PipeOps")` into data flow graphs.
...
We choose to impute factorial features using a new level (via `r ref("PipeOpImputeNewlvl")`).
Let's use the `PipeOp` itself to create an imputed `Task`.
This shows us how the `PipeOp` actually works:

```{r}
imputer = po("imputenewlvl")
...
```{r}
imp_ranger = GraphLearner$new(po("imputenewlvl") %>>% ranger)
...
```{r}
fpipe = po("imputenewlvl") %>>% po("filter", flt("mim"), filter.nfeat = 3)
...
bmr = benchmark(data.table(
  ...
  learner = list(
    stackgraph,
    GraphLearner$new(po("imputenewlvl") %>>% lrn("classif.ranger")),
    GraphLearner$new(po("imputenewlvl") %>>% lrn("classif.kknn")),
    GraphLearner$new(po("imputenewlvl") %>>% lrn("classif.log_reg"))),
...
To solve this issue we can use `r ref("PipeOpImputeSample")`, but with `affect_cols` set to only *factorial* features.
