...
This is the third part in a serial of use cases with the German Credit data.
...
We use the **mlr3pipelines** that enables us to chain "`PipeOp`" objects into data flow graphs. 
...
We do not only need complete data for modeling but also for prediction.
...
Typically, sparse models, i.e. having models with few(er) features, is desirable.
This is due to a variety of reasons.
Some of them are an enhanced interpretability or decreased costs of acquiring data.
...
It makes use of a F-test for values in different target classes (equivalent to a t-test in the binary classification case).
...
Making use of these filter, you may wonder at which costs the reduction of the feature space comes.
We can investigate the trade-off between features and performance by tuning.
We incorporate our filtering strategy in the pipeline using the `"filter"` `PipeOp`.
...
Thus, we make use of grid search. 
...
While in this setting sparisty looks desirable, there should be at least two features which are used.
...
This is a very specific case which is luckily handled by the `"learner_cv"` PipeOp.
...
We compare the perfomance of the stacked learner to performance of the individual `Learner`s. 
The stacked learner, the random forest and the logistic regression seem to have negligable difference in their cross-validated `mmce` only.
...
Prediction then fails because the `Learner` does not know how to handle unseen factor levels.
...
There are many more technical issues which may result in errors when training (or predicting) which we have not presented yet.
