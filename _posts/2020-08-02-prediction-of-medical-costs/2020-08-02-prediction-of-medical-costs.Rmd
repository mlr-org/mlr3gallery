...
plot_charges <- function(col, title, lab){
  ggplot() +
  geom_point(data = insurance, aes(x = col, y = charges), col = "grey30") +
  ggtitle(title) +
  xlab(lab) +
  theme_bw() +
...
plot_facet <- function(facet, title, ncol = 1) {
  insurance1 <- insurance %>% mutate(facet = facet)
  ggplot() +
  geom_histogram(data = insurance1, aes(x = charges), col = "darkgray", size = 0.5) +
...
grid.arrange(plot_charges(insurance$age, "age", "age standardized"),
             plot_charges(insurance$bmi, "bmi", "bmi standardized"),
             plot_charges(insurance$children, "children", "children standardized"),
             plot_facet(insurance$sex, "sex"),
             plot_facet(insurance$smoker, "smoker"),
             plot_facet(insurance$region, "region", ncol = 2),
             nrow = 2)
```
