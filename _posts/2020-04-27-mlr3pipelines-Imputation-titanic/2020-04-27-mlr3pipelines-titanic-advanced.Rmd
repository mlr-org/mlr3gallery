...
Luckily, this is automatically handled in our `r ref("Graph")`, if we use it through a `GraphLearner`.
...
Now we can put everything together again, we concatenate our new `r ref("PipeOp")` with the `r ref("Graph")` created above and use `PipeOpSelect` in order to de-select the `character` features we used for feature extraction.
