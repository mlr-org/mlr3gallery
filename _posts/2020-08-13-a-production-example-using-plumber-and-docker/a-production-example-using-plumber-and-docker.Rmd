...
```{r a-production-example-using-plumber-and-docker-002}
library(data.table)
library(mlr3)
...
```{r a-production-example-using-plumber-and-docker-003}
data = tsk("boston_housing")$data()
data = data[, c("medv", "crim", "tax", "town")]
```

```{r a-production-example-using-plumber-and-docker-004}
...
```{r a-production-example-using-plumber-and-docker-005}
g = po("imputemedian") %>>%
  po("imputeoor") %>>%
  po("fixfactors") %>>%
  lrn("regr.rpart")
...
```{r a-production-example-using-plumber-and-docker-006}
...
```{r a-production-example-using-plumber-and-docker-007, eval=FALSE}
gl$model
```

Furthermore, we can save the trained pipeline, i.e., as `"gl.rds"`:

```{r a-production-example-using-plumber-and-docker-008, eval=FALSE}
saveRDS(gl, "gl.rds")
```
...
```{r a-production-example-using-plumber-and-docker-009, eval=FALSE}
feature_info = list(
...
```{r a-production-example-using-plumber-and-docker-010, eval=FALSE,code=readLines("mlr3_api/train_gl.R")}
```
...
```{r a-production-example-using-plumber-and-docker-011}
...
```{r a-production-example-using-plumber-and-docker-012, eval=FALSE,code=readLines("mlr3_api/fix_feature_types.R")}
```
...
```{r a-production-example-using-plumber-and-docker-013, eval=FALSE,code=readLines("mlr3_api/predict_gl.R")}
```

Note that the only difference to a regular R file is the comment

```{r a-production-example-using-plumber-and-docker-014, eval=FALSE}
#* @post /predict_medv`
```
...
```{r a-production-example-using-plumber-and-docker-015, eval=FALSE}
library(plumber)
...
```{r a-production-example-using-plumber-and-docker-016, engine="bash",eval=FALSE}
curl --data '[{"crim":3.14, "tax":691, "town":"Newton"}]' "http://127.0.0.1:1030/predict_medv"
```

This should return the predicted `medv`:

```{r a-production-example-using-plumber-and-docker-017, engine="bash",eval=FALSE}
[{"row_id":1,"response":"32.2329"}]
```

Alternatively, we can also use the `httr::POST` function within R:

```{r a-production-example-using-plumber-and-docker-018, eval=FALSE}
...
```{r a-production-example-using-plumber-and-docker-019, eval=FALSE}
newdata = '[
...
```{r a-production-example-using-plumber-and-docker-020, engine="bash",eval=FALSE}
...
```{r a-production-example-using-plumber-and-docker-021, engine="bash",eval=FALSE}
...
```{r a-production-example-using-plumber-and-docker-022, engine="bash",eval=FALSE}
RUN mkdir /data
COPY gl.rds /data
...
```{r a-production-example-using-plumber-and-docker-023, engine="bash",eval=FALSE}
EXPOSE 1030
ENTRYPOINT ["R", "-e", \
...
```{r a-production-example-using-plumber-and-docker-024, engine="bash",eval=FALSE,code=readLines("mlr3_api/Dockerfile")}
```

To build the image we open a terminal in the `mlr3_api` directory and run:

```{r a-production-example-using-plumber-and-docker-025, engine="bash",eval=FALSE}
...
```{r a-production-example-using-plumber-and-docker-026, engine="bash",eval=FALSE}
...
```{r a-production-example-using-plumber-and-docker-027, engine="bash",eval=FALSE}
