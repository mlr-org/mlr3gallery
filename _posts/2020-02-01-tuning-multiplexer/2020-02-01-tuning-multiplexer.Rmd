...
glrn$param_set$values$branch.selection = to_tune(levels = c("rf", "xgb"))
...
search_space = ps(
  branch.selection = p_fct(levels = c("rf", "xgb")),
  rf.mtry = p_int(lower = 1L, upper = 20L, depends = branch.selection == "rf"),
  xgb.nrounds = p_int(lower = 1, upper = 500, depends = branch.selection == "xgb"))
```
...
glrn = GraphLearner$new(pipe, id = "g")
...
```{r 2020-02-01-tuning-multiplexer-017, results = 'hide'}
rr = tune_nested(
  method = "grid_search",
  task = task,
  learner = glrn,
  inner_resampling = cv1,
  outer_resampling = cv2,
  measure = msr("classif.ce"),
  search_space = search_space,
  term_evals = 10L
)
