...
library(mlr3book)
library(rmarkdown)
library(mlr3tuning) # mlr3verse CRAN update for `tune()`
```
...
```{r practical-tuning-series-tune-a-support-vector-machine-0137777, echo = FALSE, eval = params$eval_all}
saveRDS(instance, "data/instance_1.rda")
```

```{r practical-tuning-series-tune-a-support-vector-machine-013, echo = FALSE}
instance = readRDS("data/instance_1.rda")
```

We plot the performances depending on the evaluated `cost` and `gamma` values.

```{r practical-tuning-series-tune-a-support-vector-machine-015, eval = FALSE}
...
```{r practical-tuning-series-tune-a-support-vector-machine-018}
learner = lrn("classif.svm", type = "C-classification", kernel = "radial")
...
```{r practical-tuning-series-tune-a-support-vector-machine-0137778, echo = FALSE, eval = params$eval_all}
saveRDS(instance, "data/instance_2.rda")
```

```{r practical-tuning-series-tune-a-support-vector-machine-020, echo = FALSE}
...
```{r practical-tuning-series-tune-a-support-vector-machine-023}
library(ggplot2)
library(scales)
...
```{r practical-tuning-series-tune-a-support-vector-machine-030, eval = params$eval_all}
instance = tune(
  method = "grid_search",
  task = task,
  learner = learner,
...
```{r practical-tuning-series-tune-a-support-vector-machine-0137779, echo = FALSE, eval = params$eval_all}
saveRDS(instance, "data/instance_3.rda")
```

```{r practical-tuning-series-tune-a-support-vector-machine-029, echo = FALSE}
...
```{r practical-tuning-series-tune-a-support-vector-machine-035, eval = params$eval_all}
resampling_outer = rsmp("cv", folds = 3)
rr = resample(task = task, learner = at, resampling = resampling_outer, store_models = TRUE)
```

```{r practical-tuning-series-tune-a-support-vector-machine-0137771, echo = FALSE, eval = params$eval_all}
...
```{r practical-tuning-series-tune-a-support-vector-machine-040, eval = FALSE}
learner = lrn("classif.svm", type = "C-classification")
learner$param_set$values$cost = to_tune(p_dbl(1e-5, 1e5, logscale = TRUE))
learner$param_set$values$gamma = to_tune(p_dbl(1e-5, 1e5, logscale = TRUE))
learner$param_set$values$kernel = to_tune(c("polynomial", "radial"))
