...
```{r mlr3fselect-basic-02743, echo = FALSE}
...
```{r practical-tuning-series-tune-a-support-vector-machine-014, eval = params$eval_all}
tuner$optimize(instance)
```

```{r practical-tuning-series-tune-a-support-vector-machine-015, echo = FALSE, eval = params$eval_all}
saveRDS(instance, "data/instance_1.rda")
```

```{r practical-tuning-series-tune-a-support-vector-machine-016, echo = FALSE}
instance = readRDS("data/instance_1.rda")
```

We plot the performances depending on the evaluated `cost` and `gamma` values.

```{r practical-tuning-series-tune-a-support-vector-machine-017, eval = FALSE}
autoplot(instance, type = "surface", cols_x = c("cost", "gamma"), 
  learner = lrn("regr.km"))
```

```{r practical-tuning-series-tune-a-support-vector-machine-018, echo = FALSE}
...
```{r practical-tuning-series-tune-a-support-vector-machine-019, eval = FALSE}
learner = lrn("classif.svm", type = "C-classification", kernel = "radial")
learner$param_set$values$cost = to_tune(0.1, 10)
learner$param_set$values$gamma = to_tune(0, 5)
...
```{r practical-tuning-series-tune-a-support-vector-machine-022, eval = params$eval_all}
instance = tune(
  method = "grid_search",
  task = task,
  learner = learner,
...
```{r practical-tuning-series-tune-a-support-vector-machine-023, echo = FALSE, eval = params$eval_all}
saveRDS(instance, "data/instance_2.rda")
```

```{r practical-tuning-series-tune-a-support-vector-machine-024, echo = FALSE}
...
```{r practical-tuning-series-tune-a-support-vector-machine-025, eval = FALSE}
as.data.table(instance$archive)
```

```{r mlr3fselect-basic-027, echo = FALSE}
...
```{r practical-tuning-series-tune-a-support-vector-machine-032, eval = params$eval_all}
instance = tune(
  method = "grid_search",
  task = task,
  learner = learner,
...
```{r practical-tuning-series-tune-a-support-vector-machine-033, echo = FALSE, eval = params$eval_all}
saveRDS(instance, "data/instance_3.rda")
```

```{r practical-tuning-series-tune-a-support-vector-machine-034, echo = FALSE}
...
```{r practical-tuning-series-tune-a-support-vector-machine-038, eval = params$eval_all}
resampling_outer = rsmp("cv", folds = 3)
rr = resample(task = task, learner = at, resampling = resampling_outer, store_models = TRUE)
```

```{r practical-tuning-series-tune-a-support-vector-machine-039, echo = FALSE, eval = params$eval_all}
saveRDS(rr, "data/rr_1.rda")
```

```{r practical-tuning-series-tune-a-support-vector-machine-040, echo = FALSE}
...
```{r practical-tuning-series-tune-a-support-vector-machine-041, eval = FALSE}
extract_inner_tuning_results(rr)
```

```{r mlr3fselect-basic-0273453, echo = FALSE, layout="l-body-outset"}
...
```{r practical-tuning-series-tune-a-support-vector-machine-042, eval = FALSE}
rr$score()
```

```{r mlr3fselect-basic-0272, echo = FALSE}
...
```{r practical-tuning-series-tune-a-support-vector-machine-044}
rr$aggregate()
```
